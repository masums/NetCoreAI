...
- [~] Multi-tenant mode: tenant resolver, per-tenant models/KBs/agents/quotas/storage paths — *isolation, resolvers and storage paths done; the tenant is part of every key and every query filter. **Quotas and the dashboard tenant switcher are outstanding.** Needs a database reset to upgrade, because the primary keys change — see [docs/guides/multi-tenancy.md](docs/guides/multi-tenancy.md)*
